...
class DiagnosticContractTests(unittest.TestCase):
    ...
    def test_mesa_bridge_uses_a_fenced_completion_driven_worker(self):
        ...
        self.assertIn('"MESA_KGSL_X11_GPU_BRIDGE_MIN_PIXELS", 0', source)
        ...
        self.assertIn("state->next_target_msc = complete->msc + 2", source)
        self.assertIn("state->gpu_blit ? XCB_PRESENT_OPTION_NONE", source)
    ...
    def test_kgsl_bridge_is_default_and_safe_mode_remains_available(self):
        agent = (ROOT / "native/agent/main.cpp").read_text()
        runner = (ROOT / "native/agent/run-agent.sh").read_text()
        package_gpu = (ROOT / "build-support/package-gpu-stack.sh").read_text()
        ...
        self.assertIn('setenv("LIBGL_DRIVERS_PATH", mesa.c_str(), 1);', agent)
        self.assertIn('setenv("GBM_BACKENDS_PATH", gbm_backends.c_str(), 1);', agent)
        self.assertIn('/lib/mesa/kgsl_dri.so', agent)
        self.assertIn('/lib/mesa/libgbm.so.1', agent)
        self.assertIn('/lib/mesa/gbm/dri_gbm.so', agent)
        self.assertIn('$BUNDLE/lib/mesa/kgsl_dri.so', runner)
        self.assertIn('$BUNDLE/lib/mesa/libgbm.so.1', runner)
        self.assertIn('$BUNDLE/lib/mesa/gbm/dri_gbm.so', runner)
        self.assertIn('libdril_dri.so', package_gpu)
        self.assertIn('ln -s libdril_dri.so "$STAGE/lib/mesa/kgsl_dri.so"', package_gpu)
        self.assertIn('libgbm.so.1', package_gpu)
        self.assertIn('gbm/dri_gbm.so', package_gpu)
