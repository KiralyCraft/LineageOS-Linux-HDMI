...
expected_binary_commit = sys.argv[6] if len(sys.argv) > 6 else None
required = {
...
assert len(info["binary_repository_commit"]) == 40
assert info["binary_reuse_verified"] == (
    info["binary_repository_commit"] != info["package_repository_commit"]
)
if expected_binary_commit:
    assert info["binary_repository_commit"] == expected_binary_commit
