...
python "$SOURCE/build-support/verify-reused-build.py" \
    "$BASE_INFO" "$BASE_BUILD" "$BASE_COMMIT"

rm -rf -- "$DEST_BUILD"
mkdir -p "$DEST_BUILD"
cp -a "$BASE_BUILD/composer" "$BASE_BUILD/native" "$BASE_BUILD/tile" \
    "$DEST_BUILD/"
cp -a "$BASE_BUILD/exact-source-sync.json" "$BASE_BUILD/qcom-display.patched" \
    "$BASE_BUILD/composer-abi-report.txt" "$DEST_BUILD/"

python "$SOURCE/build-support/verify-reused-build.py" \
    "$BASE_INFO" "$DEST_BUILD" "$BASE_COMMIT"
