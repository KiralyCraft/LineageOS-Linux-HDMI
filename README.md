...
`libGLX_mesa.so.0`, `libEGL_mesa.so.0`, `libdril_dri.so`, `libgbm.so.1`, and
`gbm/dri_gbm.so` below `lib/mesa/` in the chroot runtime bundle. `kgsl_dri.so`
is a relative link to that private DRIL target. All six Mesa binaries must come
from the same completed build.
...
Mesa's standard `LIBGL_DRIVERS_PATH` and `GBM_BACKENDS_PATH` to select the
private KGSL and GBM targets, checks
