...
| `kgsl-kms-bridge` (default) | Native Freedreno/KGSL, zero-copy Xorg scanout, and a pipelined MIT-SHM copy per swapped accelerated drawable | The allocation/presentation bridge produced visible LXDE and `glxgears`; release `0.2.8-candidate.2` adds stricter mode and scanout gates, and preserves arming across the required unplug, but still requires target-device validation |
